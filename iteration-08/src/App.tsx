...
export const App: FC<AppProps> = () => {
  ...
  const { steps, currStepIndex, currStep, firstForm, lastForm, back, next } =
    FormsHandler([
      <OrderSummary key={'summary'} cart={cartData} />,
      <RecipientSelection key={'recipient'} users={userData} />,
      <BillingInfoForm
        key={'billing'}
        {...billingData}
      ...
      <form onSubmit={onSubmit}>
        ...
        >
          ...
          >
            ...
            <button type="submit" style={{ fontFamily: 'Brush Script MT' }}>
              {' '}
              {lastForm ? 'Finish' : firstForm ? 'Gift to' : 'Next'}{' '}
            </button>
          </div>
        </div>
      </form>
